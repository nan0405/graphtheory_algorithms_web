...
//   if (pseudo.includes("t := get(min(Dist")) {
...
//   if (pseudo.includes("Xét các lân cận của")) {
...
async function runDijkstra() {
  ...
  // Kiểm tra node bắt đầu có tồn tại không
  ...
  try {
    ...
    // Chỉnh sửa URL gọi API: Thêm tham số 'end' để thuật toán dừng khi tìm thấy đích
    let url = `${SERVER_BASE}/api/graphs/${id}/teach?start=${startInput}`;
    if (endInput && nodes.find(n => n.id === endInput)) {
      url += `&end=${endInput}`;
    }
    ...
    // document.getElementById('content-wrapper').classList.add('explain-open');
    // document.getElementById('explanation-wrapper').classList.add('show-steps');
...
const STEP_TO_PSEUDO = [
  ...
  { match: "w(v) := ∞", key: "init-dist" },
  { match: "] := 0", key: "init-start" },
  { match: "Prev", key: "init-prev" },

  { match: "while Open", key: "loop" },

  { match: "Chọn đỉnh", key: "pick-min" },
  { match: "loại khỏi Open", key: "move" },
  { match: "Thêm vào Close", key: "add-close" },
  { match: "x = z", key: "check-end" },

  { match: "Xét đỉnh kề", key: "for" },
  { match: "chưa thuộc Close", key: "for" },

  { match: "chưa có trong Open", key: "if1" },
  { match: "Thêm vào Open", key: "if11" },
  { match: "w[", key: "if12" },
  { match: "Prev[", key: "if13" },
];
