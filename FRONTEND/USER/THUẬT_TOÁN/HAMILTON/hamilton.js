...
async function applyStepVisuals(stepIndex) {
  ...
  if (pseudo.includes("Thêm")) {
    ...
    for (const e of highlightEdges) {
      ...
      // ✅ LƯU TRẠNG THÁI VÀO PATH
      persistedBlueEdges.add(id);
      persistedBlueEdges.add(reversed);
      persistedBlueNodes.add(a);
      persistedBlueNodes.add(b);
  ...
  if (pseudo.includes("Bỏ qua")) {
    for (const e of highlightEdges) {
      const id = e.toLowerCase();
      const reversed = id.split("-").reverse().join("-");
      const targetNodeId = id.split("-")[1];

      const nodeInPath = persistedBlueNodes.has(targetNodeId);
      const edgeInPath =
        persistedBlueEdges.has(id) || persistedBlueEdges.has(reversed);
      ...
      linkGroup
        ...
        .transition()
        .duration(300)
        .attr("stroke", "#cfd8dc")     // 🔥 xám nhạt
        .attr("stroke-width", 2)       // mỏng hơn
        .attr("stroke-dasharray", "6,4"); // nét đứt

      // NODE
      nodeGroup
        .selectAll("circle")
        .filter((d) => d.id.toLowerCase() === targetNodeId)
        .transition()
        .duration(300)
        .attr("fill", "#1976d2");
    }
...
async function applyStepVisuals(stepIndex) {
  ...
  if (pseudo.includes("Bỏ qua")) {
    ...
    for (const e of highlightEdges) {
      ...
      // =========================
      // EDGE → luôn xám + nét đứt
      // =========================
      linkGroup.selectAll("line")
        .filter(d => sameEdge(d.id, e))
        .transition()
        .duration(300)
        .attr("stroke", "#b0bec5")     // xám nhạt
        .attr("stroke-width", 2)       // mỏng hơn
        .attr("stroke-dasharray", "6,4"); // nét đứt

      // =========================
      // NODE → luôn xanh dương
      // =========================
      nodeGroup.selectAll("circle")
        .filter(d => d.id.toLowerCase() === targetNodeId)
        .transition()
        .duration(300)
        .attr("fill", "#1976d2")       // xanh chuẩn
        .attr("stroke", "#0d47a1")
        .attr("stroke-width", 6);
    }
  }

  if (pseudo.includes("Đang tại")) {

    const currentNodeId = highlightNodes?.[0]?.toLowerCase();
    if (!currentNodeId) return;

    nodeGroup.selectAll("circle")
      .filter(d => d.id.toLowerCase() === currentNodeId)
      .each(function () {

        const node = d3.select(this);

        // 🔵 Lưu trạng thái gốc
        const originalFill = node.attr("fill") || "#1976d2";
        const originalStroke = node.attr("stroke") || "#0d47a1";
        const originalStrokeWidth = node.attr("stroke-width") || 6;

        node
          .transition()
          .delay(2500)                 // 🔥 đợi 2.5s trước khi sáng
          .duration(500)
          .attr("fill", "#42a5f5")     // sáng hơn
          .attr("stroke", "#1e88e5")   // viền sáng hơn
          .attr("stroke-width", 8)     // viền đậm hơn chút
          .transition()
          .duration(500)
  ...
  nodeGroup.selectAll("circle").each(function (d) {
    ...
    const safeCurrent =
      typeof currentVertex !== "undefined" && currentVertex
        ? currentVertex.toLowerCase()
        : null;

    if (
      persistedBlueNodes.has(id) &&
      id !== safeCurrent
    ) {
      d3.select(this)
        .attr("fill", "#1976d2")
...
async function runDijkstra(startNode) {
  ...
  const payload = {
    ...
    Edges: links.map(l => {
      const { s, t } = getEndpoints(l);
      return { From: s, To: t, Weight: Number(l.weight || 1) };
    }),
    Directed: false,
  };

  // 1️⃣ Tạo graph trên server
  ...
  const runResp = await fetch(
    `${SERVER_BASE}/api/graphs/${id}/hamilton?start=${start}`
  );

  if (!runResp.ok) {
    const err = await runResp.json();
    throw new Error(err.message || "Run Hamilton failed");
  }

  const runData = await runResp.json();

  // Hamilton cũng trả về pages giống Prim/Dijkstra
